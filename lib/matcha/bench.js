...
function runStatic (cb) {
  ...
  this.emit('start');
  timer.start();
  runner(this.fn, pending, next, timer);
}
...
function runAdaptive (cb) {
  ...
  function next (count) {
    ...
    if (timer.stop().elapsed < duration) {
      ...
      runner(self.fn, pending, next, timer, self);
    } else {
  ...
  this.emit('start');
  timer.start();
  runner(this.fn, pending, next, timer, self);
}
...
function doSync (fn, iterations, next, timer, self) {
  var i = iterations, j;

  if (self.suite.beforeEach.length) {
    while (i--) {
      timer.pause();
      self.suite.beforeEach[0]();
      timer.start();
      fn();
    }
  } else {
...
function doAsync (fn, iterations, next, timer, self) {
  function iterate (i) {
